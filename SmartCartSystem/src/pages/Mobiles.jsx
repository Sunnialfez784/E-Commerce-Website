          ...
          <section className="section-surface p-5 sm:p-6">
            <div className="flex items-end justify-between gap-4 border-b border-slate-200 pb-5">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Mobiles</h2>
              </div>
              {!loading && (
                <span className="premium-pill">{mobileData.length} {mobileData.length === 1 ? "product" : "products"}</span>
              )}
            </div>
            ...
            {loading ? (
              ...
              <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {mobileData.map((item, i) => (
                  <Cards key={i} item={item} isProduct />
                ))}
              </div>
            ) : (
              <div className="empty-state mt-6">
                ...
                <h3 className="text-base font-semibold text-slate-900">No mobiles found</h3>
