    ...
    <section className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center pb-12 pt-4">
      <div className="space-y-5 order-2 lg:order-1">
        ...
        <div className="flex flex-wrap gap-2">
          {[110, 150, 130].map((w, i) => (
            <Sk key={i} className="h-7 rounded-lg" style={{width: w}} />
          ))}
        </div>
...
function AnimatedSection({children, delay = 0}: {children: React.ReactNode; delay?: number}) {
  ...
  return (
    <div ref={ref} style={{transitionDelay: `${delay}ms`}} className={`transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
      {children}
    </div>
  );
}
...
function StatCard({value, label}: {value: number | string; label: string}) {
  return (
    <div className="flex flex-col items-center gap-1 px-3 sm:px-6 py-4 rounded-2xl bg-white border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 transition-all duration-300 group cursor-default shadow-sm">
      <span className="text-2xl sm:text-3xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors font-mono tracking-tight">{value}</span>
...
function RepoCard({repo}: {repo: GitHubRepo}) {
  return (
    <a href={repo.html_url} target="_blank" rel="noopener noreferrer" className="group flex flex-col gap-3 p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-indigo-100 h-full shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-semibold text-slate-800 group-hover:text-slate-900 transition-colors text-sm truncate">{repo.name}</span>
        </div>
        <span className="text-xs text-slate-400 shrink-0">{timeAgo(repo.updated_at)}</span>
      </div>
      {repo.description && <p className="text-xs text-slate-500 leading-relaxed line-clamp-2">{repo.description}</p>}
...
function HasnainAbout() {
  ...
  useEffect(() => {
    async function fetchGitHub() {
      try {
        const [userRes, reposRes] = await Promise.all([fetch("https://api.github.com/users/HasnainMakati"), fetch("https://api.github.com/users/HasnainMakati/repos?sort=updated&per_page=6")]);
      ...
      <div className="text-slate-800 w-full" style={{fontFamily: "'DM Sans', sans-serif"}}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16">
            ...
            <section className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center min-h-[60vh] pb-12">
              ...
              <div className="space-y-5 sm:space-y-7 order-2 lg:order-1">
                ...
                <div className="space-y-2 sm:space-y-3">
                  <h1 className="font-syne font-extrabold text-4xl sm:text-5xl xl:text-7xl leading-[0.95] tracking-tight text-slate-900">
                    {user?.name?.split(" ")[0] ?? "Hasnain"} <span className="gradient-text">{user?.name?.split(" ").slice(1).join(" ") ?? "Makati"}</span>
                  </h1>
                ...
                <p className="text-slate-600 text-sm sm:text-[15px] leading-relaxed max-w-md">{user?.bio ?? "Backend Developer with hands-on experience in Node.js, Express.js, MySQL, and Sequelize."}</p>

                {/* Meta pills */}
                <div className="flex flex-wrap gap-2">
                  {user?.location && <span className="flex items-center gap-1.5 text-xs text-slate-600 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">📍 {user?.location}</span>}
                  <span className="flex items-center gap-1.5 text-xs text-slate-600 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">💼 Open to opportunities</span>
                  <span className="flex items-center gap-1.5 text-xs text-slate-600 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">🚀 Building cool stuff</span>
                </div>

                {/* CTAs */}
                <div className="flex flex-wrap gap-3 pt-2">
                  <a href={user?.html_url ?? "https://github.com/HasnainMakati"} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-indigo-200">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                  ...
                  {user?.blog && (
                    <a href={user?.blog?.startsWith("http") ? user?.blog : `https://${user?.blog}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl bg-white border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 text-slate-700 font-semibold text-sm transition-all duration-200 hover:-translate-y-0.5 shadow-sm">
                      🌐 Portfolio
                    </a>
                  )}
                  <a href="mailto:novotrends2@gmail.com" className="flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl bg-white border border-slate-200 hover:border-emerald-300 hover:bg-emerald-50 text-slate-700 font-semibold text-sm transition-all duration-200 hover:-translate-y-0.5 shadow-sm">
                    ✉️ Contact
                  </a>
                </div>
              </div>
              ...
              <div className="flex justify-center lg:justify-end order-1 lg:order-2">
                <div className="relative float-anim">
                  {/* Spinning ring */}
                  <div className="absolute inset-[-18px] rounded-full spin-slow opacity-70" style={{background: "conic-gradient(from 0deg, #6366f1, #0891b2, #7c3aed, #6366f1)", padding: "2px"}}>
            ...
            <section className="mb-16 sm:mb-20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-6 sm:gap-8 mt-6 sm:mt-8">
                <div className="md:col-span-3 space-y-4 text-slate-600 text-sm sm:text-[15px] leading-relaxed">
                  <p>
                    I'm <span className="text-slate-900 font-semibold">Hasnain Makati</span>, a passionate Backend Developer.
                  </p>
                  <p>Backend Developer with hands-on experience in Node.js, Express.js, MySQL, and Sequelize. Skilled in building secure REST APIs, implementing JWT authentication, managing databases, integrating Cloudinary for file uploads, and Razorpay for payment processing.</p>
                  <p>Developed real-world projects including School Management and Shopping Management systems. Passionate about writing clean, scalable code and continuously learning modern backend technologies to build efficient and reliable applications.</p>
                </div>
              ...
              <div className="mt-6 sm:mt-8 relative rounded-2xl sm:rounded-3xl border border-indigo-200 bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-6 sm:p-8 md:p-10 overflow-hidden shadow-sm">
                ...
                <div className="relative grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10">
                  <div className="space-y-4 sm:space-y-5">
                    ...
                    <a href={user?.html_url ?? "https://github.com/HasnainMakati/E-Commerce-Website"} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition-colors group">
                      View on GitHub
          ...
          <AnimatedSection>
            <section>
              <div className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-gradient-to-br from-indigo-50/60 via-white to-emerald-50/60 p-8 sm:p-10 md:p-14 text-center relative overflow-hidden shadow-sm">
                ...
                <h2 className="font-syne font-extrabold text-3xl sm:text-4xl md:text-5xl text-slate-900 mb-3 sm:mb-4 leading-tight">Got a project in mind?</h2>
                <p className="text-slate-500 text-sm sm:text-base mb-6 sm:mb-8 max-w-lg mx-auto">I'm always open to discussing new opportunities, interesting projects, or just chatting about tech.</p>
                <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4">
                  <a href="mailto:novotrends2@gmail.com" className="flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-3.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-indigo-200">
                    ✉️ Send me an email
                  </a>
                  <a href={user?.html_url ?? "https://github.com/HasnainMakati"} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-3.5 rounded-xl bg-white border border-slate-200 hover:border-indigo-300 text-slate-700 font-semibold text-sm transition-all duration-200 hover:-translate-y-0.5 shadow-sm">
                    🐙 GitHub Profile
                  </a>
                </div>
              </div>
          ...
          <footer className="mt-12 sm:mt-16 pt-6 sm:pt-8 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-3 text-slate-400 text-xs font-mono-dm">
            <span>
              © {new Date().getFullYear()} {user?.name ?? "Hasnain Makati"}. All rights reserved.
            </span>
