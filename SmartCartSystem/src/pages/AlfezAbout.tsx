...
const TECH_STACK = [
  ...
  { name: "Python",       icon: "🐍", color: "from-green-50 to-green-100 border-green-300 text-green-700"},
    ...
    async function fetchGitHub() {
      try {
        const [userRes, reposRes] = await Promise.all([
          fetch("https://api.github.com/users/Sunnialfez784"),
          fetch("https://api.github.com/users/Sunnialfez784/repos?sort=updated&per_page=6"),
        ]);
      ...
      <div className="text-slate-800 w-full" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16">
              ...
              <div className="space-y-5 sm:space-y-7 order-2 lg:order-1">
                ...
                <div className="space-y-2 sm:space-y-3">
                  <h1 className="font-syne font-extrabold text-4xl sm:text-5xl xl:text-7xl leading-[0.95] tracking-tight text-slate-900">
                    {user?.name?.split(" ")[0] ?? "Alfez"}{" "}
                    <span className="gradient-text">
                      {user?.name?.split(" ").slice(1).join(" ") ?? "Sunni"}
                    </span>
                  </h1>
                ...
                <p className="text-slate-600 text-sm sm:text-[15px] leading-relaxed max-w-md">
                  {user?.bio ?? "Passionate Frontend developer crafting scalable web applications with the Frontend."}
                </p>
            ...
            <section className="mb-16 sm:mb-20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-6 sm:gap-8 mt-6 sm:mt-8">
                <div className="md:col-span-3 space-y-4 text-slate-600 text-sm sm:text-[15px] leading-relaxed">
                  <p>
                    I'm <span className="text-slate-900 font-semibold">Sunni Alfez</span>, a passionate Frontend Developer. I enjoy building scalable web applications using React and TailwindCss. I love solving real-world problems through clean and efficient code.
                  </p>
                  <p>
          ...
          <footer className="mt-12 sm:mt-16 pt-6 sm:pt-8 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-3 text-slate-400 text-xs font-mono-dm">
            <span>© {new Date().getFullYear()} {user?.name ?? "Alfez Sunni"}. All rights reserved.</span>
