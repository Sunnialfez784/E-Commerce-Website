...
const Navbar = ({isButton}) => {
  ...
  const navLinkClass = ({isActive}) =>
    `inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-colors duration-200 whitespace-nowrap sm:gap-2 sm:px-4 sm:py-2 sm:text-sm ${
      isActive
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/60 bg-white/85 text-black shadow-[0_12px_40px_-20px_rgba(15,23,42,0.35)] backdrop-blur-xl">
      <nav className="page-shell py-3 sm:py-4">
        <Header />

        <div className="mt-3 rounded-[28px] border border-slate-200 bg-white/90 px-2 py-2 shadow-sm sm:mt-5 sm:px-3 sm:py-2.5">
          <div className="relative flex items-center gap-1">
            ...
            <button
              ...
              className={`shrink-0 flex h-7 w-7 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 shadow-sm transition-all duration-200 sm:h-8 sm:w-8 ${
                canScrollLeft
            ...
            <ul
              ref={scrollRef}
              className="flex flex-1 items-center gap-1 overflow-x-auto font-medium sm:gap-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
            >
